...
const LINES = [
  {
    productCode: "A5900AA.",
    description: "Ideal Standard Thermostatic Mixing Valve 15Mm (Under Basin) Chrome",
    qty: 1,
    unitPrice: 119.17,
  },
  {
    productCode: "A4852(AA)",
    description: "Sensorflow 21 Compact Mixer",
    qty: 1,
    unitPrice: 476.18,
  },
];
...
async function main() {
  const po = await prisma.customerPO.findFirst({
    where: { poNo: "0088/110204" },
    include: { ticket: true, lines: true },
  });
  if (!po) throw new Error("PO not found");
  if (po.lines.length) {
    console.log(`PO already has ${po.lines.length} line(s) — aborting to avoid duplicates`);
    return;
  }
  ...
  for (const l of LINES) {
    const tl = await prisma.ticketLine.create({
      data: {
        ...
        actualSaleTotal: l.unitPrice * l.qty,
      },
    });
  ...
  console.log(`\n2 PO lines created · total £${total.toFixed(2)} (matches PO limit £${po.poLimitValue})`);
}
