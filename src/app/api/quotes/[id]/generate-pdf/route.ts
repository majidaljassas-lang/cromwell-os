...
function buildHtml(quote: {
  ...
  const lineRows = quote.lines.map((line, i) => {
    ...
    return `${sectionHeader}<tr style="border-bottom:${isBom ? "none" : "1px solid #eee"};${rowStyle}">
      ...
      <td style="padding:12px 10px;font-size:13px;font-weight:500">${line.description}${isFoc ? focBadge : ""}</td>
      <td style="padding:12px 10px;text-align:center;font-size:13px">${Number(line.qty)} ${line.ticketLine?.unit || "LOT"}</td>
      <td style="padding:12px 10px;text-align:right;font-size:13px;font-variant-numeric:tabular-nums;${priceStyle}">${priceCell}</td>
      <td style="padding:12px 10px;text-align:right;font-size:13px;font-variant-numeric:tabular-nums;font-weight:600;${priceStyle}">${totalCell}</td>
    </tr>${bomRows}`;
  }).join("");
